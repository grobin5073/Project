...
/*function CommaFormatted(amount) {
	...
	var delimiter = ","; 
	var i = parseFloat(amount);//parseInt(amount);
...
function applyName(klass, numStrings) {

    var toAdd = $("td." + klass).text();
    
    var actualClass = $("td." + klass).attr("rel");
    
    $("input." + actualClass).attr("value", numStrings + " strings");
...
function calcTotalStrings() {

    var totalStrings = 0;

    $(".num-strings-input").each(function() {
    
        var thisValue = parseInt($(this).val());
    
        if ( (IsNumeric(thisValue)) &&  (thisValue != '') ) {
            totalStrings += parseInt(thisValue);
        };
    
    });
    
    $("#total-strings-input").val(totalStrings);
...
function calcProdSubTotal() {
    ...
    $("#product-subtotal").val(prodSubTotal);  //(CommaFormatted(prodSubTotal));

}

function calcShippingTotal() {

    var totalStrings = $("#total-strings-input").val() || 0;
    var shippingRate = $("#shipping-rate").text() || 0;
    var shippingTotal = totalStrings * shippingRate;
    
    $("#shipping-subtotal").val(shippingTotal);//(CommaFormatted(shippingTotal));
...
function calcOrderTotal() {
    ...
    var orderTotal = parseFloat(productSubtotal) + parseFloat(shippingSubtotal); //parseInt(CleanNumber(productSubtotal)) + parseInt(CleanNumber(shippingSubtotal));    
        
    $("#order-total").val(orderTotal);//(CommaFormatted(orderTotal));
...
$(function() {
    ...
    $('.num-strings-input').bind("focus blur change keyup", function(){
        ...
        // If the quantity is valid, calculate the row total
        } else if ( (IsNumeric(numStrings)) && (numStrings != '') ) {
            
            var rowTotal = numStrings * multiplier;
            ...
            applyName(titleClass, numStrings);
